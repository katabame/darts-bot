...
def calculate_score(scores: list[list[str]]) -> tuple[int, str]:
    total_score: int = 0
    score_text: str = ''
    ...
    for score in scores:
        if score[1] == 'BULL':
            ...
            multiplier: int = 3 if score[0] == 'I' else 2 if score[0] == 'T' else 1
            score_text += f'{"O" if score[0] in ["", "S", "O"] else score[0]}{score[1]} '
...
def generate_stats(current_score:int, scores: list[list[str]], total_rounds:int, field_value: list[str]):
    ...
    current_round = 1
    score_added = False
    round_score, score_text = calculate_score(scores)
